...
export default class UpdateFoodModal extends Component {
  constructor(props) {
    super(props);
    this.state = {isShowModal: false, nameFood: null, priceFood: 0};
  }

  showModal = (item, food, index) => {
  ...
  _onSave = (name, price) => {
    ...
    foodData[index].name = name;
    foodData[index].price = price;
    this.state.item.refreshItem();
    ...
    return (
      <View>
        <Modal
          ...
          <View>
            <Text style={styles.text_title}>Edit Food</Text>
            <TextInput
